...
- `BYPASS_ENTRY_SCORE_GATING=true` bypasses CHOP/regime and live score-floor entry blockers so each round can attempt paired entry, while orderbook, timing, pair-cost, queue, participation, cooldown, and exit/hedge rules still apply.
- `BYPASS_SINGLE_FILL_COOLDOWN=true` bypasses the active single-fill cooldown entry blocker. It does not disable the single-fill hedge/profit-exit logic.
